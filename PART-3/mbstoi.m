function [ sii ] = mbstoi(xl, xr, yl, yr, fs_signal)
% -----------------------------------------------------------------------------%
%                               Inputs                                         %
% -----------------------------------------------------------------------------%
...
%                               Description                                    %
% -----------------------------------------------------------------------------%
% This is an implementation of the Modified Binaural Short-Time
...
%                                   Licence                                    %
% -----------------------------------------------------------------------------%
% License Agreement for the MBSTOI software (the ”Software”)
% 
...
%                               Parameters                                     %
% -----------------------------------------------------------------------------%
fs          = 10000;    % Sample rate of proposed intelligibility measure [Hz].
...
tau_min         = -0.001;% Minumum interaural delay compensation [s]. B: -0.01.
tau_max         = 0.001; % Maximum interaural delay compensation [s]. B: 0.01.
...
sigma_epsilon_0 = 1.5;      % ILD compensation standard deviation [-].
...
if fs_signal ~= fs
    xl = resample(xl, fs, fs_signal);
	xr = resample(xr, fs, fs_signal);
    yl = resample(yl, fs, fs_signal);
    yr = resample(yr, fs, fs_signal);
end

% Remove silent frames
[xl, xr, yl, yr] = remove_silent_frames(xl, xr, yl, yr, dyn_range, ...
                                        N_frame, N_frame/2);

% Handle case when signals are zeros
if abs(log10(norm(xl)/norm(yl)))>5.0||abs(log10(norm(xr)/norm(yr)))>5.0
    sii = 0;
    return
end
...
p_ec_max = zeros(J, length(N:size(xl_hat, 2)));
...
sigma_epsilon = sqrt(2)*sigma_epsilon_0*(1+(abs(gammas)./alpha_0_db).^p)/20;
gammas = gammas/20;
sigma_delta = sqrt(2) * sigma_delta_0 * (1 + (abs(taus)/tau_0));       

% The loop below pretty much just evaluates one huge equation in every
% interation. See referenced notes for details.
for i=1:J
    for j=1:size(d,2) % Number of frames
        ...
        if(min(abs(exx.*eyy))<1e-40)
            d(i,j) = -1;
            continue;
        else
            p = exx./eyy;
            [tmp, idx1] = max(p);
            [p_ec_max(i,j), idx2] = max(tmp);
            d(i,j) = exy(idx1(idx2),idx2)/sqrt(exx(idx1(idx2),idx2)*eyy(idx1(idx2),idx2));
        end
    end
end
...
pl  	= zeros(J, length(N:size(xl_hat, 2)));
pr  	= zeros(J, length(N:size(xl_hat, 2)));

% Compute temporary better-ear correlations
for m = N:size(xl_hat, 2)
    Xl_seg  	= Xl(:, (m-N+1):m);
    Xr_seg  	= Xr(:, (m-N+1):m);
    Yl_seg  	= Yl(:, (m-N+1):m);
    Yr_seg  	= Yr(:, (m-N+1):m);
    for n = 1:J
        ...
        pl(n,m-N+1) = sum(xln.*xln)/sum(yln.*yln);
        pr(n,m-N+1) = sum(xrn.*xrn)/sum(yrn.*yrn);
...
p_be_max = max(pl, pr);
dbe_interm = zeros(size(dl_interm));
dbe_interm(pl>pr) = dl_interm(pl>pr);
dbe_interm(pr>pl) = dr_interm(pr>pl);

%% ----------------------------------------------------------------------------%
%                               Compute STOI measure                           %
% -----------------------------------------------------------------------------%
d(p_be_max>p_ec_max) = dbe_interm(p_be_max>p_ec_max);
sii = mean(d(:));
...
function  [A cf fids] = thirdoct(fs, N_fft, numBands, mn)
